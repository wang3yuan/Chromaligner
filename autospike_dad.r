...
for(i in 1:length(file_name)) 
{
 ...
 RAW.intensity[[i]] = RAW.intensity[[i]] - min(RAW.intensity[[i]]);
 RAW.intensity[[i]] = RAW.intensity[[i]] + min(RAW.intensity[[i]][RAW.intensity[[i]]>0])/10;
...
for(n in 1:nsample)
{
 ...
 write.table(cbind(RAW.intensity[[n]],r.time[[n]]),file_name[n],row.names=F,col.names=F,sep="\t")
