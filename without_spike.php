...
#make para file
	...
	$fp=fopen("method_0_temp.r","w");
	fputs($fp,$command);
	fclose($fp);
	exec("cat method_0_temp.r ../without_spike.r > without_spike.r");
	#   unzip the uploading files
	exec("unzip -ouj ".$_FILES['Samples']['name']." -d ./",$a);
	exec("chmod 777 *");
	exec("cp ../Baseline_correction_function_dad.r Baseline_correction_function_dad.r");	
	exec("/usr/bin/R --no-save < without_spike.r");
...
#	 echo "<input onclick=\"history.go(-1)\"  type=\"button\" value=\"Go Back to midify\" name=\"Go back\" style=\"color:rgb(255,255,204); background-color:rgb(255,0,102); border-color:rgb(255,0,102); border-style:none;\">";
#	 echo "01";
	}
	if($_POST['start_flag']=="yes" & $_POST['end_flag']!="yes")
	{
...
#	 echo "<br><h4>The following image is an example telling the contents in parameter files.</h4>";	 
#	 echo "<img border=1 style=\"border-width: 1px\"src=\"Spec/PPopt3_nopeaks.jpg\" /><br></br>";	 
#	 echo "11";	 
	}	
...
//			echo "rows".round($rows)."<br>";
//			echo "number of rows".$number_of_rows;
			if($number_of_rows!=$rows){
				echo "Please check the parameter file P".$line[0]." . It contains ".$rows." rows.<br>";
				echo "It should contain ".$number_of_rows." rows.<br>";
				exit;
			}else{
...
/**/			if(strcmp(str_replace(" ","\ ",$_POST['target']),str_replace(" ","\ ",$line[0]))!=0){			
				...
				//	$number_of_spikes." ".$_POST['no_datapoints']." ".$_POST['no_secs']." ".
					$eof_flag." ".$end_flag."\n");
			}
/**/			if($i==($number_of_sample-1))	
/**/			{
