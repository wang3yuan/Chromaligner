...
void Align_eof(double* data_array,int eof,int data_len);

int main(int argc,char* argv[]){
	int i,j,k,check,no_datapoints,no_secs;/*check is a receiver of return value*/
	double freq_per_sec,eof_flag,end_flag;/*check is a receiver of return value*/
	...
	freq_per_sec=(int)((double)no_datapoints/no_secs);
	max_range=max_range*60*freq_per_sec;
	p_option=0;
	if(eof_flag==-1){
		p_option++;
	}else if(eof_flag<0){
	...
	if(check){
		fprintf(stderr,"Please check sample parameter file %s and target parameter file %s.\n",argv[2],argv[1]);
		return 0;
	}
	if(eof_flag!=-1){
...
/*for(i=0;i<spike_number+2;i++){
	...
	Align_eof(target_array,target_spike[0],target_spike[spike_number+1]);
	for(i=1;i<=spike_number;i++){
		target_spike[i]-=target_spike[0];	
	}
	target_spike[0]=0;
	Align_eof(sample_array,sample_spike[0],sample_spike[spike_number+1]);
	for(i=1;i<=spike_number;i++){
		...
		for(Section_N=2;Section_N<=Lt/20;Section_N++){
			for(Slack=1;Slack<=5;Slack++){
				...
				if(temp>cor_max){
					...
					for(j=target_spike[i];j<target_spike[i+1];j++){
...
/*	for(i=0;i<max_range;i++){
	...
	fclose(fout);
	Free();
	printf("%s is aligned already.<br>\n",argv[2]);
	return 0;
	...
	for(i=strlen(Name)-1;i>=0;i--){
		if(Name[i]=='.'){
			...
			if(strcmp(name_tmp,"txt")!=0){
				fprintf(stdout,"Please check the \"input.txt\". The first colum should be \"real\" file name.<br>\n");
				fprintf(stdout,"And we only accept the txt file format.<br>\n");
				return -1;
			}
			Name[i]=0;
			break;
...
int get_data(double* intensity_array,FILE* fp,int Pcount,int max_length){
	...
	if(i<Pcount){
		fprintf(stderr,"The number of rows in file is less than the endtime for peak alignment.");
		return -1;
	}
...
void Align_eof(double* data_array,int wave_eof,int data_len){
	int i;
